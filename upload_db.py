...
LOCAL_DB_PATH = 'db.sqlite3'  # Caminho corrigido
DROPBOX_PATH = '/db.sqlite3'  # Caminho no Dropbox

# Inicializa o cliente Dropbox
dbx = dropbox.Dropbox(ACCESS_TOKEN)

# Função para fazer upload do banco
def upload_db():
    try:
        with open(LOCAL_DB_PATH, 'rb') as f:
            dbx.files_upload(f.read(), DROPBOX_PATH, mode=WriteMode('overwrite'))
        print(f"Banco de dados enviado para {DROPBOX_PATH} com sucesso!")
    except Exception as e:
        print(f"Erro ao enviar o banco: {e}")
